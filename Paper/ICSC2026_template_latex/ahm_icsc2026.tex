...
\authorrunning{AuthorA and AuthorB}
\institute{InstituteA \and InstituteB\\
...
to receive chord events scheduled into any synthesis instrument.
...
voicing styles (Four-Way Close, Drop~2, Drop~3, Drop~2+4). The pop dataset covers
...
(6~classes: Joyful, Vital, Epic, Uneasiness, Depressive, Despair),
\textbf{Scale/Mode} (7~classes: Ionian, Aeolian, Dorian, Phrygian, Lydian,
Mixolydian, Harmonic minor), and \textbf{Key} (12 major and 12 minor).
...
\texttt{gen\_data.tsv} (1{,}296 rows) maps progression IDs to concrete chord
strings across 12 pitch classes, with major/minor spellings handled according
to scale context.
...
into any target instrument (here, Instrument~2).
...
idiom~\cite{boulanger2011}. At each event, \texttt{emoChord} prints the
...
The combined dataset totals 2{,}196 rows across 108 progression types,
6~emotions, and 7~scales; to our knowledge no public harmony corpus unifies
jazz and pop under a six-class affect taxonomy.

After Optuna tuning (50 trials, 5-fold CV), the final model achieves a held-out
top-1 accuracy of~24.8\% and top-3 of~49.1\%. A uniform random baseline yields
0.9\% (1/108); a majority-class-per-pair baseline yields 24.8\%, confirming the
model has learned the dominant harmonic preference per \texttt{(emotion, scale)} cell.
The model's role is not prediction beyond this majority baseline but to package
the learned distribution as an ONNX tensor: the argmax case ($T\to 0$) reduces
to the majority lookup, while temperature sampling draws from the full distribution.

An ablation at 477~progressions (4.4$\times$) dropped top-1 to 10.6\%:
feature vectors within a cell are identical, bounded by the dominant-class
proportion within each cell, a design-time ceiling.

The 10~MB \texttt{gen\_model.onnx} bundles with the plugin for fully offline
deployment; all materials are at \url{https://github.com/CharlieSL1/AHM-Dataset}.


\section{Conclusion}

AHM demonstrates a reusable pattern for embedding ONNX-compatible classifiers
with a defined input/output tensor contract in Csound as a native i-rate opcode
via the ONNX Runtime C~API, with no Python or external-process dependency.
Within this contract, substituting a different classifier requires no plugin
source changes.

For the composer, affect becomes a score parameter; temperature spans from
near-deterministic to open-ended harmonic exploration.

Future directions include a genre input to resolve jazz/pop cell ambiguity,
longer chord sequence generation, a free-text prompt interface mapping
natural-language input to the nearest \texttt{(emotion, scale)} pair, a k-rate
variant, an \texttt{iKey} parameter, a perceptual evaluation, and Windows/Linux
porting. Dataset and model improvements are ongoing; these results are an
initial validated release. AHM establishes that small, domain-specific
ML models are practical candidates for native Csound integration, offering a
template for AI-augmented composition without cloud dependency.
